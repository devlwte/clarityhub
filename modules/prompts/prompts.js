...
// UtilCode
const utilcode = require("../utilcodes");
...
class Prompts_ {
    ...
    async compressIMG(urlimg, { update = false, saveIn, name, saveTo, ...arg }) {
        if (urlimg !== false) {
            ...
            if (saveTo == "_base64") {
                ...
                // verificar si existe
                if (!update) {
                    if (fs.existsSync(path.join(saveIn, `${name}.jpg`))) {
                        return true;
                    }
                }
                ...
                try {
                    ...
                    if (getbuffer) {
                        // Compress
                        const optimizedImageBuffer = await this.optimizeImage(getbuffer);
                        await utilcode.fsWrite(path.join(saveIn, `${name}.jpg`), optimizedImageBuffer);
